...
def import_data(infile, outfile):
    ...
    casa_command = "importvla(archivefiles = {0}, vis = '{1}')".format(data_files, msfile)
    logger.info('Executing command: '+ casa_command)

    cgatcore_command = ('''udocker run --volume="$(pwd)":/data casa --nogui --logfile casa.log'''+
                        ''' -c "{}"'''.format(casa_command))
    P.run(cgatcore_command, job_condaenv="udocker")

    casa_command = "listobs(vis='{}', listfile='{}')".format(msfile, listobs_file)
    cgatcore_command = ('''udocker run --volume="$(pwd)":/data casa --nogui --logfile casa.log'''+
                        ''' -c "{}"'''.format(casa_command))
    P.run(cgatcore_command, job_condaenv="udocker")
...
def cleanup(infile, outfile):
    """
    Just clean working directory with all output generated by the pipeline
    """
    statement = '''rm -rf AW* *.log ctmp* *.last step.*'''
    P.run(statement)
